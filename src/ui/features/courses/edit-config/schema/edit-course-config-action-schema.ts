...
export const EditCourseConfigActionSchema = z.object({
  ...
  maximumInterval: z.number().min(0).max(default_maximum_interval),
  requestRetention: z.number().min(0).max(1),
  dailyNewCardsCount: z.number().min(0).max(100),
  showAdvancedRatingOptions: z.boolean(),
});

export type EditCourseConfigActionModel = z.infer<typeof EditCourseConfigActionSchema>;